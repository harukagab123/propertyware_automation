...
WORD_TEMPLATE_PATH = Path(os.getenv("WORD_TEMPLATE_PATH", BASE_DIR / "templates" / "3DayNotice_TEMPLATE_PLACEHOLDERS.docx"))
...
GRID_ROW_SELECTORS = [
    ...
    ".x-grid-view .x-grid-item",                   # ExtJS 4+ (rows)
    ".x-grid-view table tbody tr",                 # Table fallback
]

GRID_LOADING_MASK = [
    ".x-mask-msg",                                 # common mask
    ".x-grid3-loading",                            # ext3 loading
    ".x-mask",                                     # general mask
]
...
def _wait_for_loading_to_clear(ctx: Page | Frame, timeout_ms: int = 120_000):
    ...
    try:
        ctx.wait_for_load_state("networkidle", timeout=timeout_ms)
    except PWTimeout:
        pass
    # if there is a mask, wait for it to disappear (best-effort)
    for sel in GRID_LOADING_MASK:
        try:
            if ctx.locator(sel).first.is_visible():
                ctx.locator(sel).first.wait_for(state="hidden", timeout=timeout_ms)
        except Exception:
            continue
    # small grace period
    time.sleep(0.5)
...
def find_grid_context(page: Page, timeout_ms: int = 120_000) -> Frame | Page:
    """
    Find the frame/page that contains a Propertyware grid.
    It checks the main page first, then scans all iframes.
    Returns the context (Frame or Page) where the grid lives.
    """
    ...
    for f in frames:
        try:
            url = f.url or "<no-url>"
            logger.debug(f"Checking frame: {url}")
            _wait_for_loading_to_clear(f, timeout_ms)
            if _frame_has_grid(f):
                logger.debug(f"Grid found in frame: {url}")
                return f
        except Exception as e:
            logger.debug(f"Frame check error: {e}")
            continue

    # 3) If nothing detected, still return page (callers can try generic selectors)
...
def map_headers(ctx: Page | Frame) -> dict[str, int]:
    """
    Build a map of {header_text_lower: column_index}.
    Supports ExtJS 3 (.x-grid3-hd-row) and ExtJS 4+ (.x-grid-header-ct).
    """
    headers: dict[str, int] = {}

    # First, wait a bit for any loading mask to clear
    _wait_for_loading_to_clear(ctx, 60_000)

    # Detect "no data" state (still useful to know headers if present)
    try:
    ...
    if count and count > 0:
        for i in range(count):
            try:
                ...
                continue
        if headers:
            logger.debug(f"Header map (ext3): {headers}")
            return headers

    # Try ExtJS 4+ style
    # Headers often in .x-grid-header-ct .x-column-header .x-column-header-text
    ext4_headers = ctx.locator(".x-grid-header-ct .x-column-header")
    try:
        count2 = ext4_headers.count()
    except Exception:
        count2 = 0
    ...
    if count2 and count2 > 0:
        for i in range(count2):
            try:
                ...
                continue
        if headers:
            logger.debug(f"Header map (ext4): {headers}")
            return headers

    # Fallback: generic table headers
    ...
    if count3 and count3 > 0:
        for i in range(count3):
            try:
                ...
                continue
        if headers:
            logger.debug(f"Header map (table): {headers}")
            return headers
